...
% for composite trapezoidal, the input taken is not #nodes, but # subintervals = #nodes-1
...
% slide 6.
% we want n (number of nodes) such that the infinite norm of simpson < eps (tolerance) -> max(E_{2,m})
% solve the inequality to find h, which is related to the number of nodes as h = (b-a)/m
...
% 120 from max(derivative_4(I2))
% derivative of argument of I2 is 5*4*3*2*x, and as interval in [-1,1], max is 120
...
% when I do not know answer I have to plot all the orders, but here I know trapezoidal order 2
